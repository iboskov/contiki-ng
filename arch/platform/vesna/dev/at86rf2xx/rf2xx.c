...
// local buffer for frame preparation and faster transmit
...
static int transmit(unsigned short transmit_len) {
	...
	// inform radio how many bytes will frame contain + 2 CRC bytes
	status |= vsnSPI_pullByteTXRX(rf2xxSPI, transmit_len + RF2XX_CRC_SIZE, &dummy);


	// When CRC is offloaded to the radio, we don't need to copy 2 CRC bytes,
	// because they will be generated by the radio.
	#if !RF2XX_CONF_CHECKSUM
	...
	switch (trac) {
        ...
        case TRAC_CHANNEL_ACCESS_FAILURE:
            ...
            LOG_DBG_("TRAC=collision\n");
            return RADIO_TX_COLLISION;

        default:
            LOG_DBG_("TRAC=invalid (%u)", trac);
            return RADIO_TX_ERR;
	}
...
static int read(void *buf, unsigned short buf_len) {
	...
	trac = opts.autoACK ? bitRead(SR_TRAC_STATUS) : TRAC_SUCCESS;
